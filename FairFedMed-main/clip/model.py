...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attr: torch.Tensor=None):
        identity = x
        ...
        if attr is None:
            ...
            out = self.relu(self.bn1(self.conv1(x, attr)))
            out = self.relu(self.bn2(self.conv2(out)))
            out = self.avgpool(out)
            out = self.bn3(self.conv3(out, attr))
...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x, attr=None):
        ...
        if attr is None:
            ...
            q_proj_weight=self.q_proj.weight(x, attr)
            k_proj_weight=self.k_proj.weight(x, attr)
            v_proj_weight=self.v_proj.weight(x, attr)
            out_proj_weight=self.c_proj.weight(x, attr)
            q_proj_bias=self.q_proj.bias()
            k_proj_bias=self.k_proj.bias()
            v_proj_bias=self.v_proj.bias()
            out_proj_bias=self.c_proj.bias()
...
class ModifiedResNet_GLP_OT(nn.Module):
    ...
    def forward(self, x, attr:torch.Tensor=None):
        def stem(x):
            for conv, bn in [(self.conv1, self.bn1), (self.conv2, self.bn2), (self.conv3, self.bn3)]:
                x = self.relu(bn(conv(x)))
            x = self.avgpool(x)
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attr: torch.Tensor):
        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x), attr=attr)
        return x
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attr=None):
        # return self.resblocks(x, attr=attr)
...
class ModifiedVisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attr: torch.Tensor=None):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x, attr=attr)
        x = x.permute(1, 0, 2)  # LND -> NLD
