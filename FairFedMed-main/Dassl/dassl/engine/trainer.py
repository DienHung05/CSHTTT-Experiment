...
from .utils import get_world_size, get_local_rank
...
class TrainerBase:
    ...
    def save_model_with_grad(self, filename):
        # to filter parameters that require gradients
        state_dict = {
            name: param 
        ...
        # torch.save(self.model.state_dict(), filename.replace(".pth", "_all.pth"))
    ...
    def init_writer(self, log_dir):
        return  # Thoát ngay lập tức, không làm gì cả
    ...
    def train(self, start_epoch, max_epoch, idx=-1, global_epoch=-1, is_fed=False, is_last_client=False, global_weight=None, fedprox=False, mu=0.5):
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            self.before_epoch()
            self.run_epoch(idx, global_epoch, global_weight=global_weight,fedprox=fedprox,mu=mu, is_last_client=is_last_client)
            self.after_epoch(idx, global_epoch)
...
class SimpleTrainer(TrainerBase):
    ...
    def train(self,idx=-1,global_epoch=0,is_fed=False,is_last_client=False,global_weight=None, fedprox=False, mu=0.5):
        super().train(self.start_epoch, self.max_epoch,idx,global_epoch,is_fed,is_last_client,global_weight,fedprox,mu)
    ...
    def after_epoch(self, idx=-1, global_epoch=-1):
        ...
        if meet_checkpoint_freq or last_epoch:
            if idx == -1:
                filename = os.path.join(self.output_dir, f'epoch{str(global_epoch)}.pth')  
            else:
                filename = os.path.join(self.output_dir, f'epoch{str(global_epoch)}_client{idx}.pth')  
            self.save_model_with_grad(filename)
    ...
    def test(self, split=None, is_global=False, current_epoch=0, idx=-1, global_test=False):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            batched_data = self.parse_batch_test(batch)
            if len(batched_data) >= 3:
                if len(batched_data) == 3:
                    input, label, attrs = batched_data
                    output = self.model_inference(input)
                else:
                    input, label, attrs, tgt_attr = batched_data
                    output = self.model_inference(input, tgt_attr)
                self.evaluator.process(output, label, attrs)
            else:
                input, label = batched_data[:2]
                output = self.model_inference(input)
        ...
        for k, v in results.items():
            if not isinstance(v, float) and len(v) > 0:
                continue
            tag = f"{split}/{k}"
            if not is_global:
                tag = f"{tag}/{str(idx)}"
    ...
    def model_inference(self, input, attr=None):
        if attr is not None:
            return self.model(input, attr)
        else:
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self, idx=-1, global_epoch=-1, global_weight=None, fedprox=False, mu=0.5, is_last_client=False):
        self.set_model_mode("train")
        losses = MetricMeter()
        batch_time = AverageMeter()
        data_time = AverageMeter()
